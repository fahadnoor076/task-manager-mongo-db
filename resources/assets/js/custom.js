...
function setAdminNav(nav_id) {
	...
	// de-active list
	$("> li",mainNavElem).removeClass("active");
	$("> li",mainNavElem).removeClass("opened");
	...
	var newId = "";
	newId = $("> li ul li."+nav_id,mainNavElem).attr("id");
	if(newId){
		var navElem = newId.split('#');
		// if has child, active child
		if(regex.test(newId)) {
			//var navElem = newId.split('#');
			if(nav_id = navElem[1]){
				var childElem = navElem[0]+"#"+nav_id;
				var parentElem = $("> li[id='"+navElem[0]+"']",mainNavElem);
				// open master
				parentElem.addClass("active");
				parentElem.addClass("opened");
				$("> ul",parentElem).removeClass("collapse");
				// activate child
				$("> ul li[id='"+childElem+"'] a", parentElem).addClass("active");
			}
		}else if(navElem[0] != null || navElem[0] != ''){
			var childElem = newId;
			var parentElem = $("> li[id='"+navElem[0]+"']",mainNavElem);
			// open master
			parentElem.addClass("active");
	...
	$( 'select[name=select_action]' ).on( 'change', function () {		
		...
		if(dg_action != "") {
			...
			if(dg_ajax_params.checked_ids.length > 0) {
				...
				bootbox.alert("Please select items to delete");
			}
			select_elem.val("");
