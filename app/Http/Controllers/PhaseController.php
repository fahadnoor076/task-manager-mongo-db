...
class PhaseController extends Controller {
	...
	private $_assign_data = array(
		's_title' => 'Segment Phase', // singular title
		'p_title' => 'Segment Phases', // plural title
	...
	private $_module = "phase"; // (db module name, directory name, active nav id, routing name)
	private $_pk; // (primary key of module table : extracted from module i.e: {module_name}_id)
	private $_model = "Phase"; // name of primary model
    ...
    public function index(Request $request) {
		...
		$this->_assign_data["departments"] = $this->_assign_data["departmentModel"]->ajaxListing($search_array);
    ...
    public function ajaxListing(Request $request) {
		...
		$paginated_ids = $this->_assign_data["phaseModel"]->ajaxListing($search_array);
				...
				if (is_array($paginated_id)){
					foreach($paginated_id as $paginated_id){
						...
						$records["data"][] = array(
							...
							"phaseName" => $paginated_id['phaseName'],
							"isActive" => $status,
    ...
    private function _searchParams($request, $search_array) {
        ...
        if($request->departmentId != "") {
			$q = trim($request->departmentId);
			//$roleId = array("ObjectId" => new \MongoId($q));
			$search_array['fkDepartmentId'] = new \MongoId($q);
		}
		
		// - user designation name
		if($request->phaseName != "") {
			$q = trim($request->phaseName);
			$roleregex = new \MongoRegex("/^$q/i");
			$search_array['phaseName'] = $roleregex;
		}
    ...
    private function _add(Request $request) {
        // filter params
		$request->departmentId = trim($request->departmentId);
		$request->phaseName = trim($request->phaseName);
		...
		if ($request->departmentId == "") {
		...
		}elseif ($request->phaseName == "") {
			$field_name = "phaseName";
			$this->_json_data['focusElem'] = "input[name=" . $field_name . "]";
			$this->_json_data['targetElem'] = "div[id=error_" . $field_name . "]";
			//$this->_json_data['text'] = "Please enter Role Name";
			...
			// set record
			$save['fkDepartmentId'] = new \MongoId($request->departmentId);
			$save['phaseName'] = $request->phaseName;
			$slug = str_slug($request->phaseName, '_');
			$save['phaseSlug'] = $slug;
			...
			//INSERT
			$designation_id = $this->_assign_data['phaseModel']->_addRecord($save);
    ...
    public function update(Request $request, $id) {
		...
		// get record
		$this->_assign_data["data"] = $this->_assign_data['phaseModel']->getUpdateRecord($id);
    ...
    private function _update(Request $request, $id) {
       // filter params
		...
		if ($request->departmentId == "") {
			...
			// update
			$designation_id = $this->_assign_data['phaseModel']->_updateRecord($id, $save);
