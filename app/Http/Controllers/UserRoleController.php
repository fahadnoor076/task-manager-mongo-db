...
class UserRoleController extends Controller {
	...
	private $_assign_data = array(
		's_title' => 'User Role', // singular title
		'p_title' => 'User Roles', // plural title
	...
	private $_module = "user-role"; // (db module name, directory name, active nav id, routing name)
	private $_pk; // (primary key of module table : extracted from module i.e: {module_name}_id)
	private $_model = "UserRole"; // name of primary model
    ...
    public function ajaxListing(Request $request) {
		...
		$paginated_ids = $this->_assign_data["userRoleModel"]->ajaxListing($search_array);
       ...
       if(isset($_REQUEST['length'])){
        ...
        if (!empty($paginated_ids)) {
            ...
            foreach ($paginated_ids as $key => $paginated_id) {
                ...
                $records["data"][] = array(
                    "ids" => $checkbox,
					"userRoleName" => $paginated_id['userRoleName'],
                    "isActive" => $status,
    ...
    private function _searchParams($request, $search_array) {
		// search
        // - admin group
        if($request->userRoleName != "") {
			$q = trim($request->userRoleName);
			$roleregex = new \MongoRegex("/^$q/i");
			$search_array['userRoleName'] = $roleregex;
		}
    ...
    private function _add(Request $request) {
        // filter params
		$request->userRoleName = trim($request->userRoleName);
		...
		if ($request->userRoleName == "") {
			...
			// set record
			$save['userRoleName'] = $request->userRoleName;
			$save['isActive'] = $request->isActive;
			$slug = str_slug($request->userRoleName, '_');
			$save['userRoleSlug'] = $slug;
			$save["addedAt"] = date("Y-m-d H:i:s");
			$save["updatedAt"] = '';
			$save["deletedAt"] = '';
			
			//INSERT
			$record_id = $this->_assign_data['userRoleModel']->_addRecord($save);
			...
			/*if(isset($request->modules[0])) {
				...
				for($i=0;$i<count($request->modules);$i++) {
					...
					if($request->parents[$i] > 0) {
						$save['admin_module_id'] = $request->parents[$i];
						$save['view'] = 1;
						$save['add'] = $save['update'] = $save['delete'] = 0;
						$this->_assign_data["admin_module_permission_model"]->updatePermission($save['admin_group_id'], $save['admin_module_id'], $save);
					}
					unset($save);
				}
			}*/
    ...
    public function update(Request $request, $id) {
		...
		//GetRecordModel
		$this->_assign_data['userRoleModel']->getUpdateRecord($id);
		
		
		// get record
		$this->_assign_data["data"] = $this->_assign_data['userRoleModel']->getUpdateRecord($id);
    ...
    private function _update(Request $request, $id) {
		...
		if ($id > 1 && $request->userRoleName == "") {
			...
			$save = (array)$id;
			// set record
			$save['userRoleName'] = $id > 1 ? $request->userRoleName : $save["userRoleName"];
			$save['isActive'] = $request->isActive;
			$slug = str_slug($request->userRoleName, '_');
			$save['userRoleSlug'] = $slug;
			...
			// update
			$record_id = $this->_assign_data['userRoleModel']->_updateRecord($id, $save);
			//$this->_model->set($save[$this->_pk], $save);
			
			
			// remove permissions
			#$this->_assign_data["admin_module_permission_model"]->removePermissions($data->{$this->_pk});
			// update permissions
			/*if(isset($request->modules[0])) {
				unset($save);
				for($i=0;$i<count($request->modules);$i++) {
					...
					$save["created_at"] = date("Y-m-d H:i:s");
					// update permission
					//$this->_assign_data["admin_module_permission_model"]->updatePermission($save['admin_module_id'], $request->modules[$i], $save);
					$this->_assign_data["admin_module_permission_model"]->put($save);
					// insert parent permission
					if($request->parents[$i] > 0) {
						$save['admin_module_id'] = $request->parents[$i];
		...
		if($check_access) {
			...
			if(isset($request->delete_ids[0])) {
				...
				foreach($request->delete_ids as $id) {
					// get
					$record = $this->_assign_data["userRoleModel"]->getUpdateRecord($id);
					// if invalid record
					if($record) {
						// skip master admin
						if($record[$id]['userRoleSlug'] == 'super-admin') {
							continue;
						}
						
						$record->deletedAt = date("Y-m-d H:i:s");
						$this->_assign_data["userRoleModel"]->_updateRecord($id,(array)$record);
						$i_removed++;
					}
				}
				// msgs
    ...
    private function _selectActions($request) {
        ...
        if($request->select_action != "" && isset($request->checked_ids[0])) {
			foreach($request->checked_ids as $checked_id) {
				$record = $this->_assign_data["userRoleModel"]->getUpdateRecord($checked_id);
				// if valid record
				if($record !== FALSE) {
					// if delete
					if($request->select_action == "delete") {
						$this->_assign_data["userRoleModel"]->_updateRecord($checked_id, array('deletedAt' => date('Y-m-d H:i:s')));
