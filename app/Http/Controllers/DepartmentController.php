...
class DepartmentController extends Controller {
	...
	private $_assign_data = array(
		's_title' => 'Department', // singular title
		'p_title' => 'Departments', // plural title
	...
	private $_module = "department"; // (db module name, directory name, active nav id, routing name)
	private $_pk; // (primary key of module table : extracted from module i.e: {module_name}_id)
	private $_model = "Department"; // name of primary model
    ...
    public function ajaxListing(Request $request) {
		...
		$paginated_ids = $this->_assign_data["departmentModel"]->ajaxListing($search_array);
       ...
       if(isset($_REQUEST['length'])){
        ...
        if (!empty($paginated_ids)) {
            ...
            foreach ($paginated_ids as $key => $paginated_id) {
                ...
                $records["data"][] = array(
                    "ids" => $checkbox,
					"departmentName" => $paginated_id['departmentName'],
					"isActive" => $status,
    ...
    private function _searchParams($request, $search_array) {
        ...
        if($request->departmentName != "") {
			$q = trim($request->departmentName);
			$roleregex = new \MongoRegex("/^$q/i");
			$search_array['departmentName'] = $roleregex;
		}
    ...
    private function _add(Request $request) {
        // filter params
		$request->departmentName = trim($request->departmentName);
		...
		if ($request->departmentName == "") {
			...
			// set record
			$save['departmentName'] = $request->departmentName;
			$slug = str_slug($request->departmentName, '-');
			$save['departmentSlug'] = $slug;
			...
			//INSERT
			$record_id = $this->_assign_data['departmentModel']->_addRecord($save);
    ...
    public function update(Request $request, $id) {
		...
		// get record
		$this->_assign_data["data"] = $this->_assign_data['departmentModel']->getUpdateRecord($id);
    ...
    private function _update(Request $request, $id) {
		...
		if ($request->departmentName == "") {
			...
			// update
			$record_id = $this->_assign_data['departmentModel']->_updateRecord($id, $save);
		...
		if($check_access) {
			...
			if(isset($request->delete_ids[0])) {
				...
				foreach($request->delete_ids as $id) {
					// get
					$record = $this->_assign_data["departmentModel"]->getUpdateRecord($id);
					// if invalid record
					if(!empty($record)) {
						// skip master admin
						$record->deletedAt = date("Y-m-d H:i:s");
						$this->_assign_data["departmentModel"]->_updateRecord($id,(array)$record);
						$i_removed++;
					}
				}
				// msgs
    ...
    private function _selectActions($request) {
        ...
        if($request->select_action != "" && isset($request->checked_ids[0])) {
			foreach($request->checked_ids as $checked_id) {
				$record = $this->_assign_data["departmentModel"]->getUpdateRecord($checked_id);
				// if valid record
				if($record !== FALSE) {
					// if delete
					if($request->select_action == "delete") {
						$this->_assign_data["departmentModel"]->_updateRecord($checked_id, array('deletedAt' => date('Y-m-d H:i:s')));
