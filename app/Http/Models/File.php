...
class File extends Base implements AuthenticatableContract,
                                    ...
                                    CanResetPasswordContract
{
    ...
    //protected $hidden = ['password', 'rememberLoginToken'];
	 ...
	 public function __construct() {
        ...
        // set fields
        //
        $this->__fields = array($this->primaryKey, 'fileName', 'fkProjectId', 'fkSegmentId', 'fkPhaseId', 'fkTaskId', 'fkAddedById', 'fileType', 'isActive', 'addedAt', 'updatedAt', 'deletedAt');
    }
     ...
     */
	 ...
	 function _updateRecordByToken($token, $save){
		$mongo = mongoInitialization();
		$db = MASTER_DB_NAME;
		$table = $this->__table;
		$newdata = array('$set' => $save);
		$mongo->$db->$table->update(array("fkPostId" => $token), $newdata);
		// set pk
		return $token;
	 }
	...
	function getRecordByTaskId($taskId){
		...
		$search_array = array("fkTaskId" => new \MongoId($taskId), 'deletedAt'=>'');
		$pipeline = array(
			array(
				'$match' => $search_array,
			),
