...
class UserPrivilegeStatus extends Eloquent implements AuthenticatableContract,
                                    ...
                                    CanResetPasswordContract
{
     ...
     * @var string
     */
    public $table = 'stm_userPrivilegeStatus';
     ...
     */
	 ...
	 public function __construct() {
        ...
        // set fields
        //
        $this->__fields = array($this->primaryKey, 'designationId', 'previlegeId', 'permission', 'isActive', 'addedAt', 'updatedAt', 'deletedAt');
    }
    ...
    function checkAccess($privilege, $designationId) {
		...
		$search_array = array(
			...
			"isActive" => "1");
		$cursor = $mongo->$db->stm_userPrivilege->find($search_array);
     ...
     * @param int $group_id
     */
    function checkPrivilegeAuth($privilege, $designationId = 1) {
		if ($this->checkAccess($privilege, $designationId) === FALSE) {
	...
	function removePermissions($designationId) {	
		...
		if(($cursor->count()) > 0) {
			...
			foreach($raw_ids as $key => $raw_id) {
				$mongo = mongoInitialization();
				$db = MASTER_DB_NAME;
				$mongo->$db->$table->remove(array('_id' => new \MongoId($key)));
			}
		}
		return $return;
	}
