// default constants
if(typeof app_rel_url == undefined) { var app_rel_url = ''; }
if(typeof ajax_call_timeout == undefined) { var ajax_call_timeout = 10000; }
if(typeof ajax_response_silent == undefined) { var ajax_response_silent = 0; }
...
function flashColor(id,fadeColor,fadeSpeed) {
    ...
    if(container.length) {
		...
		fadeSpeed,'linear',function(){ $(this).animate({ backgroundColor:originalColor });});
	}
}
...
function jsonValidate(params,formElem,spinnerElem) {
	var form = null;
	...
	$.ajax({
		...
		url:app_rel_url+params, //defaults to window.location
...
function setAdminNav(activeNav) {
	var reg = new RegExp( '\\b-\\b' );
	var m_nav = activeNav;
	var s_nav = '';
	if(reg.test(activeNav)) {
		var nav = activeNav.split('-');
		m_nav = nav[0];
		s_nav = activeNav;
	}
	// close all master groups
	$('.page-sidebar-menu li:has(ul li)').removeClass('open');
	// inactive all masters
	$('.page-sidebar-menu li').removeClass('active');
	// activate current li
	$('.page-sidebar-menu li[id='+m_nav+']').addClass('active');
	
	if(s_nav != "") {
		// deativate all sub-menus
		$('.page-sidebar-menu li ul.sub-menu li').removeClass('active');
		// activate current sub-menu
		$('.page-sidebar-menu li ul.sub-menu li[id='+s_nav+']').addClass('active');
	}
}
