...
class PLPost2{wildcard_ucword}Controller extends Controller { 
    ...
    public function types(Request $request) {
		...
		if(!in_array("{plugin_identifier}/types", $this->_plugin_config->webservices)){
			...
			// other params
			$request->order_by = isset($request->order_by) ? $request->order_by : "title"; // default name
			$request->sorting = isset($request->sorting) ? strtolower($request->sorting) : "asc"; // default name
