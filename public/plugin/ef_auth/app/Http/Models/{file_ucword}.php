...
class {wildcard_ucword} extends Base {
     ...
     * @var string
     */
    public $table = '{wildcard_identifier}';
    ...
    protected $hidden = ['password', 'remember_login_token'];
	 ...
	 public function __construct() {
		...
		$this->_entity_salt_pattren = config('pl_{wildcard_identifier}.SALT');
		$this->_entity_model = $this;

        // set fields
        $this->__fields = array($this->primaryKey,'name','first_name','last_name','user_name','email','password','dob','gender','image','thumb','mobile_no','status','country_id','state_id','city_id','location','latitude','longitude','platform_type','platform_id','device_udid','device_type','device_token','is_verified','verified_at','verification_token','is_mobile_verified','mobile_verification_code','mobile_verified_at','is_guest','additional_note','other_data','remember_login_token', 'remember_login_token_created_at', 'forgot_password_hash', 'forgot_password_hash_created_at','last_login_at','last_seen_at','created_at','updated_at','deleted_at');
    }
    ...
    function setRememberToken($data) {
		...
		// set cookie
		setcookie($this->_entity_session_identifier."remember_login_token", $remember_login_token, time() + (config('pl_{wildcard_identifier}.REMEMBER_COOKIE_TIME')), "/");
		// return token
    ...
    function removeRememberToken($data) {
		if($data) {
			...
			// forget cookie
			setcookie($this->_entity_session_identifier."remember_login_token", FALSE, time() - (config('pl_{wildcard_identifier}.REMEMBER_COOKIE_TIME')), "/");
		}
	}
    ...
    function generateNew($data) {
        ...
        // temporary password
		$data->password = str_random(config('pl_{wildcard_identifier}.FORGOT_PASS_HASH_LENGTH') / 2);
    ...
    function forgotPassword($data) {
		...
		// get code
		$code = str_random(config('pl_{wildcard_identifier}.FORGOT_PASS_HASH_LENGTH'));
	...
	* @return Query
	*/
	function replaceToken($entity_id = 0, $device_token = "") {
	
		if (intval($entity_id) > 0) {
			if (trim($device_token) != "") {
			// query records
			$sql = "UPDATE `".$this->_entity_identifier."`
			SET device_token = ''
			WHERE device_token = '" . trim($device_token) . "'
			AND ".$this->_entity_pk." <> '" . $entity_id . "'
			";
			\DB::statement($sql);
			}
		}
	...
	*/
	function getData($id = 0) {
		...
		if($data !== FALSE) {
			// unset unrequired
			unset($data->deleted_at,$data->password, $data->is_verified, $data->verification_token, $data->is_guest, $data->additional_note, $data->other_data, $data->remember_login_token, $data->remember_login_token_created_at, $data->forgot_password_hash, $data->forgot_password_hash_created_at, $data->last_login_at);
		}
		return $data;
	}
